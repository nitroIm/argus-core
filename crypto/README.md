...
8. `.github/workflows/crypto_collect.yml` — cron

**Готов начинать по команде «поехали».**
...
1. Скопируй содержимое выше в файл crypto/README.md в репо
2. Закоммить через GitHub
3. Когда чат закончится — открой новый, скопируй «Промпт для нового чата» из README и вставь туда
4. Новый AI сразу поймёт контекст и продолжит с Фазы 1

---

🚦 Что делать сейчас

1. Создай crypto/README.md с этим содержимым → Commit
2. Проверь, что всё устраивает — если что-то поправить, скажи
3. Командуй «поехали» — и я начну Фазу 1 (config.py → schema.sql → db.py)

README — это твой «сохранитель игры». Не потеряешь контекст. 🦉