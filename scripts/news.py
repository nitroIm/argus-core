# ============================================================
# ARGUS — АНАЛИЗ НОВОСТЕЙ (v4)
# v4: Retries, взвешенный сентимент, дедупликация, pathlib, timezone, расширенные источники
# ============================================================
...
# ============================================================
# ИСТОЧНИКИ RSS (Приоритет: от самых надёжных к нишевым)
# ============================================================
FEEDS = [
...
# ============================================================
# ВЗВЕШЕННЫЙ СЕНТИМЕНТ (Вес > 1 усиливает влияние)
# ============================================================
BULLISH_WEIGHTS = {
    ...
    "прорыв": 1.5, "принятие": 1.2, "одобрен": 1.5, "партнёрство": 1.2, "запуск": 1.0, "позитив": 1.0,
}

BEARISH_WEIGHTS = {
    "crash": 2.0, "dump": 1.5, "plunge": 1.5, "collapse": 1.5, "fear": 1.2,
    "drop": 1.0, "fall": 1.0, "low": 1.0, "panic": 1.5, "negative": 1.0, "warning": 1.2,
    "ban": 1.5, "hack": 1.5, "scam": 1.5, "fraud": 1.5, "lawsuit": 1.2, 
    "sec": 1.2, "regulation": 1.2, "liquidation": 1.2, "bankruptcy": 1.5, "bear": 1.2,
    "падение": 1.0, "медвежий": 1.2, "обвал": 1.5, "провал": 1.2, "запрет": 1.5, 
    "взлом": 1.5, "мошенничество": 1.5, "иск": 1.2, "регуляц": 1.2, "ликвидац": 1.2, 
    "банкротств": 1.5, "паник": 1.5, "негатив": 1.0, "предупрежд": 1.2,
}
...
def translate_cached(text: str) -> str:
    ...
    if re.search(r"[а-яА-ЯёЁ]", text): return text # Уже русский
...
def fetch_feed(feed):
    ...
    try:
        ...
        # Ищем блоки item или entry (Atom)
        blocks = re.findall(r"<item\b[^>]*>(.*?)</item>", text, re.IGNORECASE | re.DOTALL)
        if not blocks:
            blocks = re.findall(r"<entry\b[^>]*>(.*?)</entry>", text, re.IGNORECASE | re.DOTALL)

        for b in blocks[:25]: # Берём топ-25 свежих с каждого источника
            # Извлекаем title, очищая от CDATA и HTML-тегов
            title_match = re.search(r"<title\b[^>]*>(.*?)</title>", b, re.IGNORECASE | re.DOTALL)
            ...
            if len(title) > 15: # Игнорируем мусорные заголовки
                items.append({
                    "title": title,
                    "source": feed["name"],
                    "lang": feed["lang"],
...
def analyze_sentiment(title):
    ...
    # Нормализуем от -1.0 (медвежий) до 1.0 (бычий)
...
# ============================================================
def main():
    print("📰 ARGUS NEWS ANALYZER v4")
    print(f"🌐 Переводчик: {'локальный' if TRANSLATE_AVAILABLE else 'НЕДОСТУПЕН (пропуск)'}")
    print("=" * 50)

    all_news = []
    seen_hashes = set() # Для дедупликации

    for feed in FEEDS:
        items = fetch_feed(feed)
        print(f"📡 {feed['name']}: получено {len(items)} заголовков")
        ...
        for item in items:
            ...
            # Переводим только если источник англоязычный или текст явно английский
    ...
    if not all_news:
        print("❌ Новостей не собрано. Проверьте сеть или RSS-ленты.")
    ...
    result = {
        ...
        "top_bullish": sorted([n for n in all_news if n["sentiment"] > 0.2], key=lambda x: x["sentiment"], reverse=True)[:5],
        "top_bearish": sorted([n for n in all_news if n["sentiment"] < -0.2], key=lambda x: x["sentiment"])[:5],
    }
